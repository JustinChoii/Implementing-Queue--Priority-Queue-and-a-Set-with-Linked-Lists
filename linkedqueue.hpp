...
std::string LinkedQueue<T>::str() const {
    ...
    if (used > 0){
        while (temp != nullptr){
            answer << temp -> value << ":" << (temp -> next == nullptr ? "]" : ",");
            temp = temp -> next;
        }
    }

    answer << "](used=" << used << ",front=" << front << ",rear=" << rear
           << ",mod_count=" << mod_count << ")";
...
LinkedQueue<T>& LinkedQueue<T>::operator = (const LinkedQueue<T>& rhs) {
    ...
    mod_count++;
    LN* current = rhs.front;
    LN* temp = new LN (current -> value);
    for(current = current -> next; current != nullptr; current = current -> next){
        if(temp -> next != nullptr){
            ...
            temp = temp -> next = new LN (current->value);
        }
    }
    temp = temp -> next;
    if(temp != nullptr){
...
std::ostream& operator << (std::ostream& outs, const LinkedQueue<T>& q) {
    ...
    if(!q.empty()){
        outs << q.front -> value;
        typename LinkedQueue<T>::LN* temp = q.front -> next;
        while (temp != nullptr){
            outs << "," << temp -> value;
            temp = temp -> next;
        }
...
T LinkedQueue<T>::Iterator::erase() {
    ...
    if (current == nullptr){
        throw CannotEraseError("LinkedQueue::Iterator::erase Iterator cursor beyong data structure");
    }
...
std::string LinkedQueue<T>::Iterator::str() const {
    std::ostringstream answer;
    answer << ref_queue -> str() << "(current=" << current << ",expected_mod_count=" <<
